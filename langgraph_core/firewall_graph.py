...
# langgraph_core/firewall_graph.py
# ------------------------------------------------------------
# Gen-AI Firewall graph for ChainGuard+
# ------------------------------------------------------------
from concurrent.futures import ThreadPoolExecutor, as_completed
from typing import TypedDict, Optional, Dict, Any
...
from agents.response_verifier_agent import ResponseVerifierAgent
from agents.audit_chain_agent     import AuditChainAgent
from agents.attack_detection_agent import AttackDetectionAgent          # (kept for future use)
from agents.code_validation_agent import CodeValidationAgent
from agents.web_search_agent      import WebSearchAgent
from agents.intial_analysis_agent import InitialAnalysisAgent           # <-- your existing typo
from llm_utils                     import call_llm
 
# ---------- NEW: context hooks ----------
from langgraph_core.hooks          import gather_context_node, inject_context_node
 
# ---------- thresholds ----------
RISK_BLOCK_THRESHOLD      = 0.85     # ≥ → block prompt entirely
RISK_FAST_PATH_THRESHOLD  = 0.20     # <  → skip heavy verification
 
# ------------------------------------------------------------
# State definition for LangGraph
# ------------------------------------------------------------
class FirewallState(TypedDict,total=False):
    # user_id:          str                      # added for context & logging
    user_prompt:      str
    context:          Optional[dict]           # filled by gather_context_node
    ...
    blockchain_log:   Optional[bool]           # unchanged
 
 
# ------------------------------------------------------------
# Instantiate (singletons reused across calls)
# ------------------------------------------------------------
initial_analyzer  = InitialAnalysisAgent()
prompt_rewriter   = SafePromptAgent()
response_verifier = ResponseVerifierAgent()
...
# ------------------------------------------------------------
def initial_analysis_node(state: FirewallState) -> FirewallState:
    res = initial_analyzer.run(state["user_prompt"])
    state["classification"]   = res.get("classification")
    state["risk_score"]       = res.get("risk_score", 0.0)
    state["risk_reason"]      = res.get("reason", "")
    state["attack_detection"] = res.get("attack_detection", {})
    return state
...
def verify_response_node(state: FirewallState) -> FirewallState:
    """Full verification fan-out (verdict + code + web search)."""
    prompt, response = state.get("final_prompt",""), state.get("llm_response","")
 
    with ThreadPoolExecutor(max_workers=3) as ex:
        futs = {
            ex.submit(response_verifier.run, prompt, response): "verdict",
            ex.submit(code_validator.run,     prompt, response): "code",
            ex.submit(web_searcher.run,       prompt, response): "search",
        }
        for fut in as_completed(futs):
            ...
            try:
                res = fut.result()
                if task == "verdict":
                    state["verdict"] = res.get("verdict") if isinstance(res,dict) else res
                elif task == "code":
                    state.update(res)
                elif task == "search":
                    state.update(res)
            except Exception as e:
                state[f"{task}_error"] = str(e)
    return state
 
 
def fast_verify_response_node(state: FirewallState) -> FirewallState:
    """Ultra-light path when risk < 0.20."""
    state["verdict"]       = "Likely factual – low-risk"
    state["code_verdict"]  = "Skipped (low-risk)"
    state["code_fragment"] = None
    return state
...
def log_audit_node(state: FirewallState) -> FirewallState:
    """Original JSON-file AuditChain (unchanged)."""
    risk = state.get("risk_score", 0.0)
    if risk >= RISK_BLOCK_THRESHOLD or state.get("blockchain_log"):
        audit_logger.log_event(state)          # still writes NDJSON
    return state
...
def build_firewall_graph():
    ...
    # ----- context phase -------------------------------------------------
    builder.add_node("gather_context", gather_context_node)   # async
    builder.add_node("inject_context", inject_context_node)   # sync
 
    # ----- core analysis & prompt handling ------------------------------
    builder.add_node("initial_analysis",   initial_analysis_node)
    builder.add_node("rewrite_prompt",     safe_prompt_node)
    builder.add_node("passthrough_prompt", passthrough_prompt_node)
    builder.add_node("blocked",            blocked_prompt_node)
    ...
    builder.add_node("llm_call",       llm_response_node)
    builder.add_node("verify_full",    verify_response_node)
    builder.add_node("verify_fast",    fast_verify_response_node)
 
    builder.add_node("audit",          log_audit_node)
 
    # ENTRY — start with context building
    builder.set_entry_point("gather_context")
    builder.add_edge("gather_context", "inject_context")
    builder.add_edge("inject_context", "initial_analysis")
 
    # ----- branching after initial analysis ------------------------------
    builder.add_conditional_edges(
        "initial_analysis",
        lambda s: "Blocked" if (
                    s["classification"] == "Blocked" or
                    float(s.get("risk_score") or 0.0) >= RISK_BLOCK_THRESHOLD
                 )
                 else "Safe" if s["classification"] == "Safe"
                 else "Risky",
        {
            "Safe":  "passthrough_prompt",
            "Risky": "rewrite_prompt",
            "Blocked":"blocked",
        }
    )
 
    # edges for prompt -> LLM
    builder.add_edge("rewrite_prompt",     "llm_call")
    builder.add_edge("passthrough_prompt", "llm_call")
 
    # ----- fast / full verification split -------------------------------
    builder.add_conditional_edges(
        "llm_call",
        lambda s: "verify_fast" if float(s.get("risk_score") or 0.0) < RISK_FAST_PATH_THRESHOLD
                  else "verify_full",
        {
    ...
    # ----- tail ----------------------------------------------------------
    builder.add_edge("verify_fast", "audit")
    builder.add_edge("verify_full", "audit")
    builder.add_edge("blocked",     "audit")
    builder.add_edge("audit", END)
